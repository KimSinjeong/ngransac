		...
		for correspondences, gt_F, gt_E, gt_R, gt_t, K1, K2, im_size1, im_size2 in testset_loader:
			...
			for b in range(correspondences.size(0)):
				...
				if opt.fmat: 
					...
					# print("correspondences[b]:", correspondences[b][:4,:,0].T.shape)
					# print("correspondences[b]:", correspondences[b][:4,:,0].T)
					x = correspondences[b][:4,:,0].T
					mkpts0 = np.concatenate([x[:, :2], np.ones_like(x[:, 0:1])], axis=-1)
					mkpts1 = np.concatenate([x[:, 2:], np.ones_like(x[:, 0:1])], axis=-1)
					
					# Let's change mkpts' normalized points to pixel coordinates
					mkpts0 = mkpts0.dot(K1[b].T)
					mkpts1 = mkpts1.dot(K2[b].T)

					mkpts0 = mkpts0[:, :2] / mkpts0[:, 2:]
					mkpts1 = mkpts1[:, :2] / mkpts1[:, 2:]

					mkpts = np.concatenate([mkpts0, mkpts1], axis=-1)
					E, mask = pygcransac.findEssentialMatrix(
						np.ascontiguousarray(mkpts), 
						K1[b], K2[b],
						im_size1[b][1], im_size1[b][0], im_size2[b][1], im_size2[b][0],
						probabilities = [],
					...
					pts1 = correspondences[b,0:2].numpy()
					pts2 = correspondences[b,2:4].numpy()
					# print("pts1 pts2 shapes:", pts1.shape, pts2.shape)
					# exit()

					# evaluation of F matrix via correspondences
					# valid, F1, epi_inliers, epi_error = util.f_error(pts1, pts2, E.numpy(), gt_E[b].numpy(), opt.threshold)
